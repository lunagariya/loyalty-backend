...
export async function reconcileInstalledShopWebhooks(appUrl:string){
  ...
  for(const installedShop of shops){
    if(installedShop.accessToken==='replace-with-development-token'){skipped++;logger.info('webhook_reconciliation_skipped',{shopDomain:installedShop.shopDomain,reason:'seed_placeholder_token'});continue;}
  ...
  logger.info('webhook_reconciliation_completed',{shopCount:shops.length,succeeded,failed,skipped});
}
